...
package org.pentaho.di.ui.trans.steps.randomrowgenerator;
...
import org.pentaho.di.trans.steps.randomrowgenerator.RandomRowGeneratorMeta;
...
public class RandomRowGeneratorDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  private RandomRowGeneratorMeta input;

  public RandomRowGeneratorDialog(Shell parent, Object in, TransMeta transMeta, String sname ) {
    super( parent, (BaseStepMeta) in, transMeta, sname );
    input = (RandomRowGeneratorMeta) in;
  }
  ...
  public String open() {
    ...
    ColumnInfo[] colinf =
      new ColumnInfo[] {
        ...
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.FixedValue" ), ColumnInfo.COLUMN_TYPE_TEXT, false ),
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.MinValue" ), ColumnInfo.COLUMN_TYPE_TEXT, false ),
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.MaxValue" ), ColumnInfo.COLUMN_TYPE_TEXT, false ),
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.EnumValue" ), ColumnInfo.COLUMN_TYPE_TEXT, false ),
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.PrefixValue" ), ColumnInfo.COLUMN_TYPE_TEXT, false ),
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.SuffixValue" ), ColumnInfo.COLUMN_TYPE_TEXT, false ),
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.CharacterType" ), ColumnInfo.COLUMN_TYPE_CCOMBO,
          getCharacterTypeNames() ),
        new ColumnInfo(
          BaseMessages.getString( PKG, "System.Column.RegularExpress" ), ColumnInfo.COLUMN_TYPE_CCOMBO,
          getCharacterTypeNames() ),
        new ColumnInfo(
  ...
  public void getData() {
    ...
    for ( int i = 0; i < input.getFieldName().length; i++ ) {
      if ( input.getFieldName()[i] != null ) {
        ...
        String minValue = input.getMinValue()[i];
        String maxValue = input.getMaxValue()[i];
        String enumValue = input.getEnumValue()[i];
        String prefixValue = input.getPrefixValue()[i];
        String suffixValue = input.getSuffixValue()[i];
        String characterType = input.getCharacterType()[i];
        String regularExpress = input.getRegularExpress()[i];
        ...
        item.setText( col++, Const.NVL( minValue, "" ) );
        item.setText( col++, Const.NVL( maxValue, "" ) );
        item.setText( col++, Const.NVL( enumValue, "" ) );
        item.setText( col++, Const.NVL( prefixValue, "" ) );
        item.setText( col++, Const.NVL( suffixValue, "" ) );
        item.setText( col++, Const.NVL( characterType, "" ) );
        item.setText( col++, Const.NVL( regularExpress, "" ) );
  ...
  private void ok() {
    ...
    try {
      getInfo( new RandomRowGeneratorMeta() ); // to see if there is an exception
  ...
  private void getInfo( RandomRowGeneratorMeta meta ) throws KettleException {
    ...
    for ( int i = 0; i < nrfields; i++ ) {
      ...
      meta.isSetEmptyString()[i] =
        BaseMessages.getString( PKG, "System.Combo.Yes" ).equalsIgnoreCase( item.getText( 17 ) );

      meta.getValue()[i] = meta.isSetEmptyString()[i] ? "" : item.getText( 9 );
      meta.getMinValue()[i] = item.getText( 10 );
      meta.getMaxValue()[i] = item.getText( 11 );
      meta.getEnumValue()[i] = item.getText( 12 );
      meta.getPrefixValue()[i] = item.getText( 13 );
      meta.getSuffixValue()[i] = item.getText( 14 );
      meta.getCharacterType()[i] = item.getText( 15 );
      meta.getRegularExpress()[i] = item.getText( 16 );
  ...
  private String[] getCharacterTypeNames()
  {
    return new String[]{"无","UUID","姓名","手机号","地址","身份证号","随机汉字组合"};
  }
  ...
  private void preview() {
    RandomRowGeneratorMeta oneMeta = new RandomRowGeneratorMeta();
    try {
      getInfo( oneMeta );
    } catch ( KettleException e ) {
      new ErrorDialog(
